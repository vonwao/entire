		...
		if (cacheNames[permission] == undefined) {
			...
			for (var i = 0; i < list.length; i++) {
				...
				if (module) {
					...
					module.extending = module.extending || [];
					for (var j = 0; j < module.extending.length; j++) {
						if (list.indexOf(module.extending[j]) == -1) {
							list.push(module.extending[j]);
						}
					}
				}
				else {
					throw new Error("Invalid module in permission or extending set: " + list[i]);
				}
			}
